...
`;
  const closeIconSVG = `<div class = "ingredient__close-btn" ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
  </div>`;
  const html = `
...
const removeEvaluatedIngredient = function (e) {
  const removeBtn = e.target.closest(".ingredient__close-btn");
  if (!removeBtn) return;
  const targetIngredient = removeBtn.closest(".ingredient");
  const targetIngredientName = targetIngredient
    .querySelector(".ingredient__text")
    .innerText.toLowerCase();
  const targetIngredientImg =
    targetIngredient.querySelector(".ingredient__img").src;
  const ingredToEvalContainer = document.querySelector(
    ".ingredients-to-evaluate"
  );
  renderIngredientToEvaluate(
    ingredToEvalContainer,
    targetIngredientName,
    targetIngredientImg
  );
  storeIngredient(user.uid, targetIngredientName, null);
  targetIngredient.remove();
};
